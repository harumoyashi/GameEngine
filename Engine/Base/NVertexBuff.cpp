...
NVertexBuff::NVertexBuff(NVertexUV* vertices, const uint32_t size)
{
	Init(vertices, size);
}
...
NVertexBuff::NVertexBuff(const std::vector<NVertexUV>& vertices)
{
	Init(vertices);
}
...
void NVertexBuff::Init(NVertexUV* vertices, const uint32_t size)
{
	HRESULT result;
	...
	uint32_t dataSize = static_cast<uint32_t>(sizeof(NVertexUV) * size);
...
void NVertexBuff::Init(const std::vector<NVertexUV>& vertices)
{
	HRESULT result;
	...
	uint32_t dataSize = static_cast<uint32_t>(sizeof(NVertexUV) * vertices.size());
	...
	view_.SizeInBytes = dataSize;
	// 頂点1つ分のデータサイズ
	view_.StrideInBytes = sizeof(NVertexUV);
}
