...
void EnemyFactory::Create(IEnemy::EnemyType type, NVec3 pos, bool isItem)
{
	...
	case IEnemy::EnemyType::SnakeSide:
		for (uint32_t i = 0; i < snakeSideNum; i++)
		{
			...
			EnemyManager::GetInstance()->enemys_.emplace_back();
			//対応した種類に所有権持たせて生成
			EnemyManager::GetInstance()->enemys_.back() = std::make_unique<Snake>();
	...
	case IEnemy::EnemyType::CarSide:
		for (uint32_t i = 0; i < carSideNum; i++)
		{
			//とりあえず追加
			EnemyManager::GetInstance()->enemys_.emplace_back();
			//対応した種類に所有権持たせて生成
			EnemyManager::GetInstance()->enemys_.back() = std::make_unique<Car>();

			//パーティクルエミッターをマネージャーに登録
			EnemyManager::GetInstance()->enemys_.back()->AddEmitter((uint32_t)EnemyManager::GetInstance()->enemys_.size() - 1);

			//出現位置を範囲内からランダムで設定
			offset = { MathUtil::Randomf(-carSideEmitter.x,carSideEmitter.x),
				0,MathUtil::Randomf(-carSideEmitter.y,carSideEmitter.y) };

			//配置位置が中心より右か左で進行方向変える
			if (pos.x > 0)
			{
				EnemyManager::GetInstance()->enemys_.back()->Generate(
					pos + offset, MathUtil::Degree2Radian(270.0f), car.modelname);
			}
			else
			{
				EnemyManager::GetInstance()->enemys_.back()->Generate(
					pos + offset, MathUtil::Degree2Radian(90.0f), car.modelname);
			}
